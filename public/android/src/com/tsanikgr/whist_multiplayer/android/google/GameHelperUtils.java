...
class GameHelperUtils {
	...
	private final static int[] RES_IDS = {
			...
			R.string.app_misconfigured, R.string.gamehelper_license_failed
	};
	...
	static String getAppIdFromResource(Context ctx) {
		try {
			...
			new Log(null).e(ex).append("failed to retrieve APP ID").print();
	...
	static String getSHA1CertFingerprint(Context ctx) {
		try {
			...
			new com.tsanikgr.whist_multiplayer.util.Log(null).e(ex).append("ERROR: package not found").print();
			return "(ERROR: package not found)";
		} catch (NoSuchAlgorithmException ex) {
			new com.tsanikgr.whist_multiplayer.util.Log(null).e(ex).append("ERROR: SHA1 algorithm not found").print();
	...
	static String getString(Context ctx, int whichString) {
		...
		try {
			...
			new com.tsanikgr.whist_multiplayer.util.Log(null).e(ex).append("GameHelper could not found resource id #").append(resId).print();
			new Log(null).e().append("*** GameHelper could not found resource id #").append(resId).append(". ").
					append("This probably happened because you included it as a stand-alone JAR. ").
					append("BaseGameUtils should be compiled as a LIBRARY PROJECT, so that it can access ").append("its resources. Using a fallback string.");
